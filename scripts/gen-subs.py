#!/usr/bin/env python3
"""Generate SRT subtitles for EN/PL/CN from narration scripts + scene timings."""
import os

ASSETS = '/opt/aigon-x-new/products/qwen-hack/assets'

# Scene durations at 24fps
SCENES = [300, 480, 720, 360, 480, 480, 480, 720, 480, 240]  # frames
TOTAL = sum(SCENES)
FPS = 24
...
def to_srt(t):
    """Seconds -> SRT timestamp H:MM:SS,mmm"""
    h = int(t // 3600)
    m = int((t % 3600) // 60)
    s = int(t % 60)
    ms = int((t - int(t)) * 1000)
...
# Estimate per-scene timing: proportional to frame count, but also account
# for scene transitions and narration speed. Use a small offset + proportional.
offset = 0.5  # start slightly after scene begins
for lang, lines in [('en', EN), ('pl', PL), ('cn', CN)]:
    srt = []
    start = offset
    for i, (frames, text) in enumerate(zip(SCENES, lines)):
        dur = frames / FPS  # scene duration in seconds
        # End of this subtitle: slightly before next scene start
        end = start + dur - 0.3
        if end > start + 1:
            srt.append(f"{i+1}\n{to_srt(start)} --> {to_srt(end)}\n{text}\n")
        start += dur

    path = f'{ASSETS}/subs_{lang}.srt'
    with open(path, 'w') as f:
        f.write('\n'.join(srt) + '\n')
    print(f"{path}: {10} subtitles")
